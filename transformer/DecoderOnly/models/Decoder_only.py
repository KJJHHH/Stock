...
class TransformerDecoderOnly(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = nn.Embedding(num_token, d_model)
        self.pos_encoder = PositionalEncoding(d_model, dropout)
        decoder_layers = TransformerDecoderLayer(
            d_model=d_model, nhead=num_head, dim_feedforward=d_hid, dropout=dropout, batch_first=True)
        self.transformer_decoder = TransformerDecoder(decoder_layers, num_layers)
        self.linear = nn.Linear(int(d_model*num_token/10), num_class)
