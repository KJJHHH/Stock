...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=len(tsmc.trainloader)*1, gamma=0.9)     
...
tsmc.validloader, 
scheduler,
) = accelerator.prepare(model, optimizer, tsmc.trainloader, tsmc.validloader, scheduler)
...
for epoch in range(last_epoch, num_epochs):
    ...
    model.train()
    loss_train_mean = 0
    for x, y in tqdm(tsmc.trainloader): 
        x = x.permute(0, 2, 1)    
        
        optimizer.zero_grad()       
        memory, outputs = model(src=tsmc.src, tgt=x, train=True)    
    ...
    # Train loss
    loss_train_mean /= len(tsmc.trainloader)
